...
public class LoginController implements ILoginController {
    ...
    public ResponseEntity<EsResponse<JwtResponse>> login(@RequestHeader String userName, @RequestHeader String password) throws Exception {
        ...
        if(userDetails.getVehicleList()!=null && !userDetails.getVehicleList().isEmpty() && userDetails.getVehicleList().size()>0) {
        	  return ResponseEntity.ok(new EsResponse<>(IConstants.RESPONSE_STATUS_OK, new JwtResponse(token,userDetails.getUserRoles()), "authentication success"));
        }else {
        	return ResponseEntity.ok(new EsResponse<>(IConstants.RESPONSE_ADD_VEHICLE, new JwtResponse(token,userDetails.getUserRoles()), "authentication success"));
        }
    }
